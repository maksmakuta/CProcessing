...
template <typename T> std::string str(T obj) { return std::to_string(obj); }
...
template <class T, class... Args> void print(T f, Args... args) {
  std::cout << f;
  print(args...);
}
template <class T, class... Args> void println(T f, Args... args) {
  std::cout << f;
  println(args...);
}
template <class T> void printArray(const std::vector<T> &arr) {
...
int day() { return getTime().tm_mday; }

// Returns the current hour as a value from 0 - 23
int hour() { return getTime().tm_hour; }

// Returns the current minute as a value from 0 - 59
int minute() { return getTime().tm_min; }

// Returns the current month as a value from 1 - 12
int month() { return getTime().tm_mon; }

// Returns the current second as a value from 0 - 59
int second() { return getTime().tm_sec; }

// Returns the current year as an integer (2003, 2004, 2005, etc)
int year() { return getTime().tm_year + 1900; }

// Returns the number of milliseconds (thousandths of a second) since starting
// an applet
int millis() { return (int)(glfwGetTime() * 1000); }

float norm(float val, float min, float max) {
  return map(val, min, max, 0.f, 1.f);
}
float dist(float x1, float y1, float x2, float y2) {
  return dist<float>(x1, y1, 0, x2, y2, 0);
}
...
public:
  ...
  color operator=(const std::string &shex) { return color(shex.c_str()); }
...
public:
  ...
  PVector(float _x, float _y, float _z) { this->set(_x, _y, _z); }
  ...
  static PVector fromAngle(float angle) { return PVector(sin(angle), cos(angle)); }

  PVector copy() { return *this; }
  ...
  static PVector add(PVector a, PVector b) { return a.add(b); }
  ...
  static PVector sub(PVector a, PVector b) { return a.sub(b); }
  ...
  static PVector mult(PVector a, float s) { return a.mult(s); }
  ...
  static PVector div(PVector a, float s) { return a.div(s); }
  ...
  float dist(PVector p) { return dist(*this, p); }

  float dot(float _x, float _y, float _z) { return x * _x + y * _y + z * _z; }

  float dot(PVector p) { return dot(*this, p); }

  float dot(PVector a, PVector b) { return a.x * b.x + a.y * b.y + a.z * b.z; }
  ...
  PVector cross(PVector a) { return cross(*this, a); }
  ...
  static PVector random2D() {
    return PVector(random<float>(-1.f, 1.f), random<float>(-1.f, 1.f))
        .normalize();
  }
...
PVector createVector(float x, float y, float z) { return PVector(x, y, z); }

PVector createVector(float x, float y) { return createVector(x, y, 0); }
...
public:
  ...
  PShape(SHAPE_TYPE st) { this->stype = st; }
  ...
  inline void push(const PVector &v) { push(v.x, v.y, v.z); }
  ...
  color getColorF() const { return this->f; }
  ...
  color getColorS() const { return this->s; }

  bool fill() const { return isFill; }
  ...
  int size() const { return verData.size(); }

  SHAPE_TYPE type() const { return stype; }

  inline void type(SHAPE_TYPE s) { stype = s; }

  std::vector<vertex> data() const { return verData; }

  inline void data(const std::vector<vertex> &d) { this->verData = d; }
  ...
  inline void done() { verData.clear(); }
};
...
PShape createTriangleFan(PVector connectTo, PVector origin, PVector start,
                         PVector end, bool clockwise) {
...
PShape createJoint(const PolySegment &segment1, const PolySegment &segment2,
  ...
  if (jointStyle == MITER) {
    ...
    auto innerSecOpt =
        LineSegment::intersection(*inner1, *inner2, allowOverlap);
    ...
    if (jointStyle == BEVEL) {
      ...
      vertices.add(createTriangleFan(innerSec, segment1.center.b, outer1->b,
                                     outer2->a, clockwise));
    }
  }
...
PShape strokify(PShape contour, float w, int cap, int join,
  ...
  for (size_t i = 0; i + 1 < contour.size(); i++) {
    ...
    auto p1 = createVector(point1.x, point1.y, point1.z);
    auto p2 = createVector(point2.x, point2.y, point2.z);
  ...
  if (loop) {
    data.add(createJoint(lastSegment, firstSegment, join, pathEnd1, pathEnd2,
                         pathStart1, pathStart2, allowOverlap));
  } else {
    if (cap == SQUARE) {
      pathStart1 = PVector::sub(
          pathStart1, PVector::mult(firstSegment.edge1.direction(), w));
      pathStart2 = PVector::sub(
          pathStart2, PVector::mult(firstSegment.edge2.direction(), w));
      pathEnd1 = PVector::add(pathEnd1,
                              PVector::mult(lastSegment.edge1.direction(), w));
      pathEnd2 = PVector::add(pathEnd2,
                              PVector::mult(lastSegment.edge2.direction(), w));

    } else if (cap == ROUND) {
      data.add(createTriangleFan(firstSegment.center.a, firstSegment.center.a,
                                 firstSegment.edge1.a, firstSegment.edge2.a,
                                 false));
      data.add(createTriangleFan(lastSegment.center.b, lastSegment.center.b,
                                 lastSegment.edge1.b, lastSegment.edge2.b,
                                 true));
    }
  }
  ...
  for (size_t i = 0; i < segments.size(); i++) {
    ...
    if (i + 1 == segments.size()) {
      ...
      data.add(createJoint(segment, segments[i + 1], join, end1, end2,
...
public:
  PImage() {}
  PImage(const std::string &name) { this->fname = name; }
  ...
  inline void load() {
    if (loaded != true) {
      ...
      if (data) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, img[c - 1], w, h, 0, img[c - 1],
  ...
  inline void create(int w, int h, unsigned char *data, int img = GL_ALPHA) {
    if (!loaded) {
      ...
      if (data) {
        printf("ID -> %i\n", textureID);
        glTexImage2D(GL_TEXTURE_2D, 0, img, w, h, 0, img, GL_UNSIGNED_BYTE,
                     data);
        // glGenerateMipmap(GL_TEXTURE_2D);
        this->loaded = true;
      } else {
  ...
  std::string getName() { return this->fname; }
  ...
  inline void done() { glDeleteTextures(1, &textureID); }
...
inline glm::vec4 vec(const color &c) { return {c.r, c.g, c.b, c.a}; }
...
inline void draw(const PShape &s) {
  ...
  glctx.sh.uvec4("color2",
                 s.fill() ? vec(glctx.strokeColor) : vec(glctx.fillColor));
...
inline void pushMatrix() { glctx.matrices.push(glctx.matrix); }
...
inline void background(color c) { glctx.bg = c; }
inline void background(int r, int g, int b, int a) {
  background(color(r, g, b, a));
}
inline void background(int r, int g, int b) { background(r, g, b, 255); }
inline void background(int v, int a) { background(v, v, v, a); }
inline void background(int v) { background(v, 255); }
inline void noFill() {
  glctx.fillFlag = false;
...
inline void fill(int r, int g, int b, int a) { fill(color(r, g, b, a)); }
inline void fill(int r, int g, int b) { fill(r, g, b, 255); }
inline void fill(int v, int a) { fill(v, v, v, a); }
inline void fill(int v) { fill(v, 255); }
...
inline void stroke(int r, int g, int b, int a) { stroke(color(r, g, b, a)); }
inline void stroke(int r, int g, int b) { stroke(r, g, b, 255); }
inline void stroke(int v, int a) { stroke(v, v, v, a); }
inline void stroke(int v) { stroke(v, 255); }
inline void strokeWeight(float w) { glctx.strokeWidth = w; }
inline void strokeJoin(int j) { glctx.join = j; }
inline void strokeCap(int c) { glctx.cap = c; }
...
inline void vertex(float x, float y, float z = 0.0f) {
  if (glctx.builder)
    ...
    std::cerr
        << "vertex() must be inside shapeBegin() and shapeEnd() functions\n";
}
...
float bezierPoint(float a, float b, float c, float d, float t) {
  return (powf((1.0f - t), 3) * a) + (3.0 * t * pow((1.0 - t), 2) * b) +
         (3.0 * t * t * (1.0 - t) * c) + (t * t * t * d);
}

inline void bezier(float x1, float y1, float x2, float y2, float x3, float y3,
                   float x4, float y4, float dt = 0.01f) {
  glctx.tmp = PShape(LINES);
  glctx.tmp.setColorS(glctx.strokeColor);
...
inline void triangle(float x1, float y1, float x2, float y2, float x3,
                     float y3) {
  if (glctx.fillFlag) {
...
inline void quad(float x1, float y1, float x2, float y2, float x3, float y3,
                 float x4, float y4) {
  if (glctx.fillFlag) {
...
inline void rect(float x, float y, float w, float h, float rtl, float rtr,
                 float rbl, float rbr) {
  if (glctx.fillFlag) {
    ...
    glctx.tmp.add(
        rectArc(x + rbl, y + h - rbl, w, h, rbl, (3 * PI) / 2, 2 * PI)); // bl
...
inline void circle(float x, float y, float r) { arc(x, y, r, 0, TWO_PI); }
inline void ellipse(float x, float y, float w, float h) {
  arc(x, y, w, h, 0, TWO_PI);
}
inline void point(float x, float y) { circle(x, y, glctx.strokeWidth); }
inline void rect(float x, float y, float w, float h, float r) {
  rect(x, y, w, h, r, r, r, r);
}
inline void square(float x, float y, float r) { rect(x, y, r, r); }

// ======================================================================================

inline void image(PImage &img, float x, float y, float w, float h, float rtl,
                  float rtr, float rbl, float rbr) {
  if (!img.isLoaded())
    img.load();
  glctx.call = 1;
...
/* Beta */
inline void textAlignH(int h){
    glctx.textAlingH = h;
}

/* Beta */
inline void textAlignV(int v){
    glctx.textAlingV = v;
}

/* Beta */
inline void textAlign(int v, int h){
    textAlignH(h);
    textAlignV(v);
}
...
inline void textSize(int size) { glctx.curr.init(size); }
...
inline void text(std::string msg, float x, float y) {
  if(glctx.textAlingH != -1){
    float w = textWidth(msg);
    switch(glctx.textAlingH){
      case ALIGN_CENTER:
        x -= w/2;
        break;
      case ALIGN_RIGHT:
  ...
  if(glctx.textAlingV != -1){
    int h = glctx.curr.getSize();
    switch(glctx.textAlingV){
      case ALIGN_CENTER:
        y += h/2;
        break;
      case ALIGN_BOTTOM:
...
inline void fullScreen() { config |= 1 << 1; }
...
inline void err(int a, const char *b) { printf("GLFW Error %i: \n%s\n", a, b); }
inline void onKey(GLFWwindow *w, int _key, int _scancode, int _action,
                  int _mods) {
  key = _key;
...
void createWindow(){
  ...
  if (config & (1 << 1))
    glfwGetMonitorWorkarea(glfwGetPrimaryMonitor(), &screenWidth, &screenHeight,
                           &width, &height);

  window = glfwCreateWindow(width, height, winTitle.c_str(),
                            config & (1 << 1) ? glfwGetPrimaryMonitor() : NULL,
                            NULL);
}
...
#ifdef FPS
      ++fps;
      if (glfwGetTime() - fpsTime > DT) {
        double speed = (double)fps / DT;
        printf("FPS : %f\n", speed);
        std::flush(std::cout);
        fps = 0;
        fpsTime = glfwGetTime();
